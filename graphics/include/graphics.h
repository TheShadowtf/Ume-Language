...
public:
    ...
    bool BuildTextMesh(const std::string& text, float scale, std::vector<float>& outVerts, std::vector<unsigned int>& outInds) const;
...
public:
    ...
    void DrawCircle(float x, float y, float radius, float r, float g, float b, float a, int segments = 32);
    ...
    // Properties & DPI scaling
    int GetWidth() const { return width_; }
    int GetHeight() const { return height_; }
    float GetAspectRatio() const { return (float)width_ / (float)height_; }
    void GetContentScale(float& sx, float& sy) const;
    ...
    void drawCircle(float x, float y, float radius, float r, float g, float b, float a, int segments = 32) {
        DrawCircle(x, y, radius, r, g, b, a, segments);
    }
    void drawText(Font* font, const std::string& text, float x, float y, float r, float g, float b, float a = 1.0f, float scale = 1.0f) {
    ...
    void getContentScale(float& sx, float& sy) const { GetContentScale(sx, sy); }
    std::vector<float> getContentScale() const {
        float sx = 1.0f, sy = 1.0f;
        GetContentScale(sx, sy);
        return { sx, sy };
    }
    void getWindowSize(int& w, int& h) const { GetWindowSize(w, h); }
    int getWindowWidth() const { return GetWindowWidth(); }
    int getWindowHeight() const { return GetWindowHeight(); }
